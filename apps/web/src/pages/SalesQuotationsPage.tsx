...
export function SalesQuotationsPage() {
  ...
  async function handleCreate(e: FormEvent) {
    ...
    setSubmitting(true);
    try {
      await apiClient.post("/ar/quotations", {
  ...
  async function convertToOrder(id: string) {
    ...
    setError(null);
    try {
      await apiClient.post(`/ar/orders/from-quotation/${id}`);
      await load();
    } catch (err: any) {
      setError(err?.response?.data?.message ?? "Failed to convert to sales order");
    } finally {
      setBusyId(null);
    }
  }
  ...
  async function cancelQuotation(id: string) {
    ...
    setError(null);
    try {
      await apiClient.post(`/ar/quotations/${id}/cancel`);
      await load();
    } catch (err: any) {
                ...
                <tr key={q.id}>
                  ...
                  <td>
                    {(q.status === "DRAFT" || q.status === "SENT") && (
                      <>
                        <button className="secondary" disabled={busyId === q.id} onClick={() => convertToOrder(q.id)}>
                          Convert to Order
                        </button>{" "}
