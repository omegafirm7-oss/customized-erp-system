...
interface Contract {
  ...
  _count: { assignments: number; usageLogs: number };
}
...
export function EquipmentContractsPage() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      const [contractsRes, partnersRes] = await Promise.all([
        apiClient.get<Contract[]>("/equipment/contracts"),
  ...
  async function handleCreate(e: FormEvent) {
    ...
    setSubmitting(true);
    try {
      await apiClient.post("/equipment/contracts", form);
  ...
  return (
    <div>
      <div className="card">
        <h2>Equipment Rental Contracts</h2>
        {loading ? (
          <p>Loading…</p>
        ) : (
          <table>
            ...
            <tbody>
              {contracts.map((c) => (
                <tr key={c.id}>
                  <td>
                    <Link to={`/equipment/contracts/${c.id}`}>{c.code}</Link>
                  </td>
                  <td>{c.name}</td>
