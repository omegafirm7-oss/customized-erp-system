...
function orderRemaining(order: SalesOrder): number {
...
export function SalesOrdersPage() {
  ...
  const [invoiceForm, setInvoiceForm] = useState({
    issueDateTime: new Date().toISOString().slice(0, 10),
  ...
  useEffect(() => {
    load();
    apiClient.get<Partner[]>("/partners").then((res) => setPartners(res.data.filter((p) => p.partnerType !== "VENDOR")));
  ...
  async function handleCreate(e: FormEvent) {
    ...
    setSubmitting(true);
    try {
      await apiClient.post("/ar/orders", {
  ...
  async function cancelOrder(id: string) {
    ...
    setError(null);
    try {
      await apiClient.post(`/ar/orders/${id}/cancel`);
      await load();
    } catch (err: any) {
      setError(err?.response?.data?.message ?? "Failed to cancel sales order");
    } finally {
      setBusyId(null);
    }
  }
  ...
  async function submitGenerateInvoice(e: FormEvent, orderId: string) {
    ...
    setError(null);
    try {
      await apiClient.post(`/ar/orders/${orderId}/generate-invoice`, {
        issueDateTime: new Date(invoiceForm.issueDateTime).toISOString(),
              ...
              {[...orders]
                ...
                <>
                  <tr key={o.id}>
                    ...
                    <td>
                      {(o.status === "CONFIRMED" || o.status === "PARTIALLY_INVOICED") && (
                        <>
                      ...
                      <td colSpan={7}>
                        <form onSubmit={(e) => submitGenerateInvoice(e, o.id)} className="form-row">
                          <div>
                            ...
                            <input
                              type="date"
                              value={invoiceForm.issueDateTime}
                              onChange={(e) => setInvoiceForm({ ...invoiceForm, issueDateTime: e.target.value })}
                              required
                            />
                          </div>
                          <div>
