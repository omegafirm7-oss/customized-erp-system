...
interface AssignmentRef {
  ...
  rateBasis: string;
  billRate: string;
  employee: { code: string; nameEn: string };
}
...
export function TimesheetPage() {
  ...
  const load = useCallback(async () => {
    const res = await apiClient.get<TimesheetDetail>(`/manpower/timesheets/${id}`);
  ...
  async function updateEntry(entry: Entry, patch: { dayType?: string; overtimeHours?: string; hours?: string }) {
    setError(null);
    try {
      await apiClient.post(`/manpower/timesheets/${id}/entries`, {
  ...
  async function action(path: string, confirmMsg?: string) {
    ...
    setBusy(true);
    try {
      const res = await apiClient.post(`/manpower/timesheets/${id}/${path}`);
  ...
  return (
    <div className="card">
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
        <h2>
          Timesheet — <Link to={`/manpower/contracts/${timesheet.contract.id}`}>{timesheet.contract.code}</Link> · Period{" "}
          {timesheet.fiscalPeriod.periodNumber}{" "}
        ...
        <span>
          ...
          {timesheet.status === "APPROVED" && (
            <>
              ...
              <button onClick={() => action("generate-invoice", "Generate the AR draft invoice from this timesheet?")} disabled={busy}>
                Generate invoice
              </button>
            </>
          )}
          {timesheet.status === "INVOICED" && timesheet.salesInvoice && (
            <>
              Invoice {timesheet.salesInvoice.invoiceNumber ?? "(draft)"} — {timesheet.salesInvoice.status}{" "}
              {timesheet.salesInvoice.status === "CANCELLED" && (
                <button className="secondary" onClick={() => action("reopen")} disabled={busy}>
      ...
      <p style={{ color: "#667085", fontSize: 13 }}>
        W = worked · R = rest · A = absent · U = unpaid leave · L = annual leave. OT = overtime hours (billable when the
        assignment carries an OT rate; feeds payroll either way).
      </p>
            ...
            <tr>
              ...
              {assignments.map((a) => (
                <th key={a.id} colSpan={2}>
                  {a.employee.code} ({a.rateBasis.toLowerCase()})
                </th>
              ))}
            </tr>
            <tr>
